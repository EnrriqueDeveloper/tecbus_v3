...
@section('contenido')
    <div class="md:flex md:justify-center md:gap-2">
        <div class="md:w-4/12">
            <img src="{{asset('img/admin.jpg')}}" alt="Imagen">
        </div>

        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form method="POST" action="{{ route('login-admin')}}" novalidate class="mt-10">
                @csrf
                @if(session('mensaje'))
                ...
                <div class="mt-8 flex justify-center">
                    <a href="{{ route('register-admin') }}" class=" text-sky-600 text-sm">Registrarse como administrador?</a>
                </div>
            </form>
        </div>
    </div>
