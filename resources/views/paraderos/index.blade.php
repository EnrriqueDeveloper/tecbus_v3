...
<div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <a type="button" href="{{ route('paraderos.create') }}" class="bg-indigo-600 px-12 py-2 rounded text-gray-200 font-semibold 
                ...
                <table class="table-fixed w-full">
                    <thead>
                        <tr class="bg-gray-300 text-white">
                            ...
                            <th class="border px4 py-2">NOM PARADERO</th>
                            <th class="border px4 py-2">HORA ESTIMADA</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="text-align:center">{{ $paradero->id }}</td>
                            <td style="text-align:center">{{ $paradero->n_paradero }}</td>
                            <td style="text-align:center">{{ $paradero->hora_estimada }}</td>
                            <td class="border px-4 py2">
                                <div class="flex justify-center rounded-lg text-lg" role="group">
                                    <!-- boton editar -->
                                    <a href="{{ route('paraderos.edit', $paradero->id) }}" class="bg-gray-900 hover:bg-gray-400 rounded text-white font-bold py-2 px-4">Editar</a>
                                    <!-- boton borrar -->
                                    <form action="{{ route('paraderos.destroy', $paradero->id) }}" method="POST" class="formEliminar">
                                        @csrf
                                        @method('DELETE')
