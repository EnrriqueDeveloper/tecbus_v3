                ...
                <form action="{{ route('rutas.update', $ruta->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                        <div class="grid grid-cols-1">
                            <label class="uppercase md:text-sm text-xs text-gray-500 font-semibold">Numero Ruta</label>
                            <input type="text" name="num_ruta" class="py-2 px-3 rounded-lg border-2 border-purple-300 focus:outline-none focus:ring-2 focus:purple-600 w-full" placeholder="Nombre" value="{{ $ruta->num_ruta }}">
                        </div>
                    </div>


                    <div class="flex items-center justify-center h-20">
                        <a type="button" href="{{ route('rutas.index') }}" class="w-auto bg-indigo-600 hover:bg-indigo-400 rounded-lg shadow-x1 font-medium text-white px-4 py-2">Cancelar</a>
